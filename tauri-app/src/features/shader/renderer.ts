import vertSrc from './crt.vert?raw';
import fragSrc from './crt.frag?raw';
...
function updateUniforms(time: number): void {
  ...
  const isStatic = (currentState === TVState.STATIC || currentState === TVState.POWERING_ON || !targetVideo || targetVideo.readyState < 2) ? 1.0 : 0.0;
...
export function initRenderer(canvas: HTMLCanvasElement, video: HTMLVideoElement): void {
